...
export const Stats: React.FC = () => {
  ...
  return (
    <div 
      ref={sectionRef} 
      className="py-24 md:py-48 bg-[#FFD700] relative overflow-hidden text-slate-900"
    ...
    >
      ...
      >
        ...
        <div className="flex justify-center items-center space-x-2 md:space-x-6 select-none scale-[0.4] sm:scale-75 md:scale-100">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className={`text-center mb-16 md:mb-32 transition-all duration-1000 ${isAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <h2 className="text-3xl md:text-7xl font-black text-[#1e293b] tracking-tight uppercase drop-shadow-sm mix-blend-multiply">
            Features of Devcotel Engineering
          </h2>
          <div className="w-20 md:w-32 h-2 md:h-2.5 bg-slate-900 mx-auto mt-6 md:mt-8 rounded-full"></div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16 lg:gap-24 mb-16">
          {features.map((feature, idx) => (
            ...
            >
              ...
              <h3 className="text-xl md:text-2xl font-black text-[#1e293b] uppercase tracking-wider mb-4 md:mb-6 group-hover:text-blue-700 transition-colors">
                {feature.title}
              </h3>
              <p className="text-[#1e293b]/80 font-bold leading-relaxed text-base md:text-lg max-w-sm px-4">
                {feature.desc}
              </p>
            </div>
          ))}
        ...
        <div className={`flex justify-center mt-12 md:mt-20 transition-all duration-1000 delay-[1400ms] ${isAnimated ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
          <button className="px-10 md:px-16 py-4 md:py-5 bg-slate-900 hover:bg-black text-white font-black text-xs md:text-sm uppercase tracking-[0.4em] rounded-full flex items-center transition-all shadow-2xl hover:shadow-black/20 transform hover:-translate-y-2 active:scale-95">
            Platform Details
            <ChevronRight className="ml-4 w-5 md:w-6 h-5 md:h-6" />
          </button>
        </div>
      </div>
    </div>
